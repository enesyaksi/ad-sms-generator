...
const CustomerCard = ({ customer, onStartCampaign, onEdit, onDelete }) => {
    return (
        <div className="group bg-surface-light dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-800 p-5 hover:shadow-lg hover:border-primary/50 transition-all duration-300 cursor-pointer flex flex-col justify-between h-full relative overflow-hidden text-left">
            {/* Quick Actions (Edit/Delete) - Visible on hover */}
            <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <button
                    onClick={(e) => { e.stopPropagation(); onEdit(customer); }}
                    className="p-1.5 rounded-lg bg-white/80 dark:bg-slate-800/80 text-slate-400 hover:text-primary hover:bg-white dark:hover:bg-slate-700 shadow-sm transition-all"
                    title="Düzenle"
                >
                ...
                <button
                    ...
                    className="p-1.5 rounded-lg bg-white/80 dark:bg-slate-800/80 text-slate-400 hover:text-red-500 hover:bg-white dark:hover:bg-slate-700 shadow-sm transition-all"
                    title="Sil"
                >
            ...
            <div className="flex flex-col gap-4">
                {/* Logo Area */}
                <div className="w-16 h-16 rounded-full bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                    {customer.logo_url ? (
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                        <a
                            ...
                            className="text-sm text-slate-500 dark:text-slate-400 hover:underline truncate flex items-center gap-1.5"
